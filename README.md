...
### Jul 14. 2026 - Phase 6 Compaction
...
Both Budget and Snip only modify the innermost `content` string of a `tool_result` block inside a `role: "user"` message — they never delete the message, the block, the `tool_use_id`, or its matching `tool_use`, so pairing is never broken. Budget uniformly compresses every over-length string based on context utilization and result size: below 50% utilization it does nothing; between 50%–70% it truncates to ~30,000 chars; above 70% it truncates to ~15,000 chars, keeping both head and tail. Snip instead does *selective* cleanup based on whether a result is stale: once utilization passes 60%, it only inspects `read_file` and `list_files` results (safe to re-fetch), keeps the 3 most recent ones in their original oldest-to-newest order, and replaces earlier ones with a placeholder. Both apply an in-place mutation directly on the original conversation history, but Snip never auto-re-reads — a tool only gets called again later if the model actually decides it needs to.

**Tier 2: Snip**

Snip only touches `tool_result` blocks inside `role: "user"` messages, and only rewrites the innermost `content` string — e.g. the file contents `read_file` returned, or the directory listing `list_files` returned. It never re-reads automatically; instead, once context utilization passes 60%, it finds every eligible stale result, keeps the 3 most recent (in the messages array's original chronological order), and replaces earlier ones with `[Content snipped - re-read if needed]`. No extra sorting is needed here, since the `messages` array is already oldest-to-newest and candidates are collected in traversal order. The final assignment is an in-place mutation: `content` and `messages[msgIdx].content` point to the same array, so mutating `content[blockIdx].content` directly edits the original conversation history — but the `tool_result` block, its `tool_use_id`, and the matching `tool_use` are all left intact, so pairing is never broken.

**Tier 3: Microcompact**

Microcompact indiscriminately clears old tool results regardless of which tool produced them, based on time-triggering. The reason is about **prompt caching** rather than context size. Anthropic's server-side prompt cache has a TTL default around 5 minutes. If a user has been idle longer than that, the cached prefix from before their pause has almost certainly expired server-side, so their next API call is going to re-upload the full prefix regardless of what this code does.

Microcompact isn't triggered because the server auto-snips the conversation — it's the client proactively compressing history at the moment the prompt cache has likely already gone cold. Using the provider this demo targets as an example: Anthropic's prompt cache only caches the *processing result* of an identical prompt prefix — it doesn't store or modify `messages` on the client's behalf. The default TTL is 5 minutes, refreshed on a cache hit; once the cache expires, the next request must reprocess whatever prefix the client sends again. So while keeping every old tool result intact is semantically safer, it means a cache miss reprocesses far more input tokens. Microcompact's strategy is: since a long-idle old cache is very likely unusable anyway, replace low-value old `tool_result.content` with a placeholder *before* the next request, sending a smaller context and cutting token cost, latency, and context pressure. This "aggressive cleanup once the cache is cold" call is an engineering trade-off this project derived from Anthropic's documented mechanism — not something Anthropic mandates. See the official docs: [Anthropic Prompt Caching Documentation](https://platform.claude.com/docs/en/build-with-claude/prompt-caching).

This demo follows the cold-cache path — compressing the local cache indiscriminately. A separate warm-cache path also exists conceptually, aimed at keeping the client's local `messages` and the original prompt prefix untouched, and delegating compression to the server instead — avoiding the client mutating old messages and immediately invalidating a cache that's still valid. In concept, the server would clear old tool-result payloads per an edit instruction and build a compressed context/cache for reuse on subsequent requests; but the publicly documented Anthropic Context Editing API doesn't guarantee a lossless "in-place update" of an existing warm cache — the docs note that related cached prefixes may still be invalidated and trigger a new cache write. So the more accurate framing is: the warm path keeps the client's full conversation history intact, and the server is responsible for producing a compressed *effective* context — without necessarily continuing to hit the original cache entry.

> [!NOTE] Prompt Prefix  
> A prompt prefix isn't a message ID — it's the contiguous, ordered content at the start of every API request, typically `tools → system → messages`. The prompt cache decides whether to hit by comparing whether this actual content is unchanged; even if `tool_use_id` or message IDs stay the same, if any content within that prefix is modified, the corresponding cache prefix can still be invalidated.
